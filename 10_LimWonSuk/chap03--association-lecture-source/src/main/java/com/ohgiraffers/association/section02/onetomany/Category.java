...
public class Category {
    ...
    /* @OneToMany
     * - 일대다 관계르 매핑할 때 사용
     * - Category 엔티티 1(하나)가 여러 Menu N(엔티티)를 가질수 있는 관계
     *
     * [!! 주의점 !!]
     * - 일대다 단방향 매핑은
     *      현재 엔티티(Category)가 아니라 반대편 엔티티(Menu)에 외래키(FK)가 있가 때문에
     *          연관 관계 처리를 위해서 UPDATE SQL이 추가적으로 실행될 수 있다.
     *  -> 현재 엔티티의 PK와 반대편 엔티티의 FK를 직접 등록 해줘도
     *      불필요한 UPDATE SQL이 수행 된다
     *
     *  - "N+1 문제"가 발생할 수 있다
     *
     * - FetchType.LAZY 시 "N+1 문제" 가 발생할 수 있다
     *
     * - fetch = fetchType.LAZY
     *   Category Emtity만 먼저 조회 후 Menu 엔티티가 필요한 시점에 따로 조회
     *       -> select가 2회 수행
     *
    ...
    // Many 로끝나면 Lazy -> Y로 끝나면 Lazy
    // One 로끝나면 EAGER -> Y로 끝나면 EAGER
